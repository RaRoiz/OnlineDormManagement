...
test('legacy six-column slip history supports rejection and resubmission without rewriting evidence', () => {
  const f = fixture();
  f.sheets.SlipReviews = new Sheet([
    ['billId', 'slipUrl', 'decision', 'reason', 'reviewedBy', 'recordedAt'],
    ['other', 'older-url', 'REJECTED', 'old reason', 'old-user', '2026-09-01']
  ]);
  const before = structuredClone(f.sheets.SlipReviews.rows);
  assert.equal(f.c.reviewBillSlip(f.request('REJECTED')).success, true);
  assert.deepEqual(f.sheets.SlipReviews.rows.slice(0, 2), before);
  assert.equal(f.sheets.SlipReviews.rows[2][0], 'bill');
  assert.equal(f.sheets.SlipReviews.rows[2][2], 'REJECTED');
  const target = f.c.selectSlipTarget_('tenant').target;
  assert.equal(target.bill.billId, 'bill');
  f.c.recordIncomingSlip_(target, 'replacement', 'line-user');
  assert.equal(f.sheets.SlipReviews.rows[3][2], 'SUBMITTED');
  assert.equal(f.bills.rows[1][f.index.paymentStatus], 'PENDING');
});

test('new incoming slip changes the selected bill row, not the header or previous row', () => {
  const f = fixture();
  f.bills.rows[1][f.index.paymentStatus] = 'PAID';
  const target = f.c.selectSlipTarget_('tenant').target;
  assert.equal(target.bill.billId, 'older');
  f.c.recordIncomingSlip_(target, 'new-slip', 'line-user');
  assert.equal(f.bills.rows[1][f.index.paymentStatus], 'PAID');
